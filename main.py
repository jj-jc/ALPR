...
for edge in image_binarized:
    image_edge.append(cv2.Canny(edge, 300, 300))
    image_edge_canny.append(cv2.Laplacian(edge, cv2.CV_8UC1))
    image_edge_sobel.append(cv2.Sobel(edge, cv2.CV_8UC1, 1, 1, ksize=5))

##------------------------- TO COMPARE THE PREPROCESSING ---------------##
i=0;
while True:
    comparative_imag = stackImages(0.6,([image_binarized[i],image_edge[i]],[image_edge_canny[i],image_edge_sobel[i]]))
    cv2.imshow("comparativa: "+ image_filenames[i],comparative_imag)
